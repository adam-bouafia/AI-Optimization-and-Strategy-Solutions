...
class MazeGenerator:
    ...
    def _is_valid_cell(self, pos: tuple[int, int]) -> bool:
        ...
        return 0 <= rowIdx < self.maze.height and 0 <= colIdx < self.maze.width

    def _get_two_step_neighbors(
        self, maze: list[list[Any]], cell: tuple[int, int], value: str = ""
    ) -> list[tuple[int, int]]:
        ...
        return [neighbor for neighbor in neighbors if self._is_valid_cell(neighbor)]
    ...
    def recursive_division(self, x1: int, x2: int, y1: int, y2: int) -> None:
        ...
        horizontal = (
            True
            if height > width
            else (False if width != height else random.choice((True, False)))
        )
    ...
    def _draw_line(
        self, x1: int, x2: int, y1: int, y2: int, horizontal: bool = False
    ) -> int:
