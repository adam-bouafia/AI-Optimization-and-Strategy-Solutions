...
from .constants import BLACK, DARK_BLUE, WHITE, WIDTH, HEIGHT
...
class Button(Widget):
    ...
    def __init__(
        ...
        self.text_surf = font.render(self.text, True, foreground_color)
...
class Menu(Widget):
    def __init__(
        self, surface: pygame.surface.Surface, button: Button, children: list[Widget]
    ) -> None:
        self.screen = surface
        self.button = button
        ...
        self.popup_rect = pygame.Rect(
            self.x - 20, self.button.rect.bottom, self.width + 40, self.height + 20
        )
    ...
    def draw(self) -> bool:
        ...
        pygame.draw.rect(self.screen, DARK_BLUE,
                         self.popup_rect, border_radius=10)
...
class Table(Widget):
    def __init__(
        ...
        max_col_widths = [
            max(child.rect.width for child in col) for col in zip(*self.children)
        ]
...
class Popup(Widget):
    def __init__(
        ...
        if orientation == Orientation.HORIZONTAL:
            match x_align:
                case Alignment.CENTER:
                    children[0].rect.left = (self.width - content_width) // 2
                case Alignment.RIGHT:
                    children[0].rect.left = self.width - \
            ...
            match y_align:
                ...
                case Alignment.BOTTOM:
                    children[0].rect.top = self.height - \
                        self.padding - content_height
                case _:
            ...
            match y_align:
                case Alignment.CENTER:
                    children[0].rect.top = (self.height - content_height) // 2
                case Alignment.BOTTOM:
                    children[0].rect.top = self.height - \
                        self.padding - content_height
                case _:
