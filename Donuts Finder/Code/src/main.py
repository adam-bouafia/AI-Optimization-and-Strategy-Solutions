...
from .maze import GOAL, START, Maze, EVIL
...
pygame.display.set_caption("Donuts Finder AIhw1 - Adam Bouafia AiConDA Univaq")
...
def draw() -> None:
    ...
    # Draw maze legend
    texts = {
        "Start Homer Node": WHITE,
        "Visited Node": BLUE,
        "Shortest-Path Node": YELLOW,
        "Unvisited Node": WHITE,
        "Wall Node": DARK,
        "Evil Node": WHITE,
        "Target Donuts Node": WHITE,
    ...
    for text in texts:
        ...
        # Draw images for Evil, start and target node
        if text == "Evil Node":
            WINDOW.blit(EVIL, (x + 3, y + 3))
            x = 50
        elif text == "Start Homer Node":
            image_rect = START.get_rect(center=(65, top.bottom + 35))
            WINDOW.blit(START, image_rect)
        elif text == "Target Donuts Node":
...
def run_single(idx: int) -> None:
    ...
    def callback():
        ...
        state.label.rect.bottom = HEADER_HEIGHT - 10
        state.overlay = False
        # Create a results list for the plot
        result_data = [(text, {
            'explored_length': solution.explored_length,
            'path_length': solution.path_length,
            'path_cost': solution.path_cost,
            'time': solution.time * 1000  # Convert to ms
        })]

        # Show a plot with detailed numbers
        plot_results(result_data, title="Detailed Results")
...
def run_all(algo_idx: int, maze_idx: int = -1) -> None:
    ...
    def callback():
        if algo_idx + 1 < len(algo_menu.children):
            ...
            # After running all algorithms in the show_results function
            ...
            plot_results(results, title="Detailed Results for All Algorithms")
...
def plot_results(results, title="Algorithm Comparison Explored paths X time"):
    algorithms = [result[0] for result in results]
    explored_lengths = [result[1]['explored_length'] for result in results]
    times = [result[1]['time'] for result in results]

    # Define a list of custom colors and markers for each algorithm
    ...
    for i in range(len(algorithms)):
        plt.scatter(times[i], explored_lengths[i], c=custom_colors[i],
                    marker=custom_markers[i], label=algorithms[i])

    plt.xlabel('Time (ms)')  # Set x-axis label
    plt.ylabel('Explored Steps')  # Set y-axis label
