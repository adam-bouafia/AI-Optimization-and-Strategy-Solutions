...
class Maze:
    ...
    def get_cell_pos(self, pos: tuple[int, int]) -> tuple[int, int]:
        ...
        return ((y - HEADER_HEIGHT) // CELL_SIZE, (x - REMAINDER_W // 2) // CELL_SIZE)
    ...
    def generate_maze(
        self, algorithm: str, after_generation: Optional[GenerationCallback] = None
    ) -> None:
        ...
        list(self.animator.nodes_to_animate.values())[-1][
            -1
        ].after_animation = after_generation
    ...
    def _draw_rect(
        ...
        if color in (BLUE, WHITE, GREEN) or (node and node.color == YELLOW):
            pygame.draw.rect(
                surface=self.surface,
        ...
        if (n := self.maze[row][col]).cost > 1:
            image_rect = EVIL.get_rect(
                center=(x + CELL_SIZE // 2, y + CELL_SIZE // 2))
            self.surface.blit(EVIL, image_rect)
