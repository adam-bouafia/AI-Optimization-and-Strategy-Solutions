...
class AnimatingNode:
    ...
    def __str__(self) -> str:
        return (
            f"AnimatingNode: Value: {self.value}, "
...
class Animator:
    ...
    def add_nodes_to_animate(
        self, nodes: list[AnimatingNode], delay: int = 0, gap: int = 10
    ) -> None:
    ...
    def _path_animation(self, node: AnimatingNode) -> None:
        ...
        if node.progress < 0.02 * node.duration:
            ...
            size = self._ease_out_sine(
                progress, MAX_SIZE, CELL_SIZE - MAX_SIZE, duration
            )
            border_radius = 0
        ...
        if node.progress < 0.02 * node.duration:
            ...
            r = self._ease_out_sine(progress, r, r2 - r, duration)
            g = self._ease_out_sine(progress, g, g2 - g, duration)
            b = self._ease_out_sine(progress, b, b2 - b, duration)
    ...
    def _ease_out_sine(
        ...
        return (
            change_in_value *
            math.sin(time / duration * (math.pi / 2)) + starting_value
        )
