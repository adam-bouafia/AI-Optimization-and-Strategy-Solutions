...
def minimax(state: State) -> State:
    """Applies the Minimax algorithm to the game.

    Args:
        state (State): The current state of the game.

    Returns:
        State: The updated state of the game after applying the Minimax algorithm.
    """

    global noOfNodes

    # we have reached the depth limit in the tree OR a final state
    if state.depth == 0 or state.board.gameOver():
        # estimate the score for the condition
...
def alpha_beta(alpha, beta, state):
    ...
    # if we reached the last level of the tree OR we reached a winning state
    if state.depth == 0 or state.board.gameOver():
       # we calculate the state score
        state.score = state.board.estimateScore(state.depth)
        return state
